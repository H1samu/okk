...
const CACHE_NAME = 'nassr-journals-v2409261';
const CORE_ASSETS = [
  './',
  './index.html',
  './manifest.json',
...
self.addEventListener('fetch', event => {
  ...
  // journals.json: network-first, so updated tags/links show up as soon as online,
  // falling back to cache when offline.
  if(url.pathname.endsWith('journals.json')){
    event.respondWith(
      fetch(event.request)
        .then(response => {
